...
export default function BarangSegment() {
    const {setBarangSegment, listBarangSegment} = customerState()
    ...
    useEffect(() => {
        fetchDataBarangSegment();
    }, []);

    const fetchDataBarangSegment = async () => {
        try {
            const response = await axios.get('/api/barang/segment');
            setBarangSegment(response.data['data']);
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/barang/segment', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/barang/segment/' + e);
    ...
    const editData = async (data) => {
        try {
            await axios.put('/api/barang/segment/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        if (value.trim() === '') {
            return listBarangSegment;
        }

        const searchValueLowerCase = value.toLowerCase();

        return listBarangSegment.filter((item) => {
            for (let key in item) {
        ...
        <div className="h-full bg-white">
            ...
            <div className="w-full p-2">
                <div className="w-full bg-[#3da0e3] py-0.5 px-1 text-white flex flex-row">
                    ...
                    <div
                        onClick={fetchDataBarangSegment}
                        className="flex-row flex items-center gap-1 px-3 py-1 hover:bg-[#2589ce] hover:cursor-pointer"
                    >
                        <BiRefresh size={12} />
