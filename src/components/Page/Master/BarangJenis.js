...
export default function BarangJenis() {
    const {setBarangJenis, listBarangJenis} = customerState()
    ...
    useEffect(() => {
        fetchDataBarangJenis();
    }, []);

    const fetchDataBarangJenis = async () => {
        try {
            const response = await axios.get('/api/barang/jenis');
            setBarangJenis(response.data['data']);
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/barang/jenis', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/barang/jenis/' + e);
    ...
    const editData = async (data) => {
        try {
            await axios.put('/api/barang/jenis/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        if (value.trim() === '') {
            return listBarangJenis;
        }

        const searchValueLowerCase = value.toLowerCase();

        return listBarangJenis.filter((item) => {
            for (let key in item) {
