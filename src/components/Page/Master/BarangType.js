...
export default function BarangType() {
    const {setBarangType, listBarangType} = customerState()
    ...
    const fetchDataBarangType = async () => {
        try {
            const response = await axios.get('/api/barang/type');
            setBarangType(response.data['data']);
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/barang/type', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/barang/type/' + e);
    ...
    const editData = async (data) => {
        try {
            await axios.put('/api/barang/type/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        if (value.trim() === '') {
            return listBarangType;
        }

        const searchValueLowerCase = value.toLowerCase();

        return listBarangType.filter((item) => {
            for (let key in item) {
