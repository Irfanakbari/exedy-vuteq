...
export default function BarangGroup() {
    const {setBarangGroup, listBarangGroup} = customerState()
    ...
    useEffect(() => {
        fetchDataBarangGroup();
    }, []);

    const fetchDataBarangGroup = async () => {
        try {
            const response = await axios.get('/api/barang/group');
            setBarangGroup(response.data['data']);
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/barang/group', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/barang/group/' + e);
            showSuccessToast("Sukses Hapus Data");
        } catch (e) {
            showErrorToast("Gagal Hapus Data");
        } finally {
            setCloseModal(false);
            await fetchDataBarangGroup();
        }
    };

    const editData = async (data) => {
        try {
            await axios.put('/api/barang/group/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        if (value.trim() === '') {
            return listBarangGroup;
        }

        const searchValueLowerCase = value.toLowerCase();

        return listBarangGroup.filter((item) => {
            for (let key in item) {
                ...
                <div className="flex overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tr>
                            ...
                            <th className="py-2 bg-gray-100 text-left">Nama</th>
                        ...
                        {filters.map((e, index) => (
                            ...
                            >
                                <td className="text-center p-1 text-sm">{index + 1}</td>
                                <td>{e['kode']}</td>
                                <td>{e['name']}</td>
                                <td>{e['keterangan']}</td>
                            </tr>
