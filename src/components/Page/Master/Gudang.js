...
export default function Gudang() {
    const {setGudang, listGudang} = customerState()
    ...
    const fetchDataGudang = async () => {
        try {
            const response = await axios.get('/api/gudang');
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/gudang', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/gudang/' + e);
            showSuccessToast("Sukses Hapus Data");
        } catch (e) {
            showErrorToast("Gagal Hapus Data");
        } finally {
            setCloseModal(false);
            await fetchDataGudang();
        }
    };

    const editData = async (data) => {
        try {
            await axios.put('/api/gudang/' + selectedCell.kode, data).then(r => {
                        ...
                        <form onSubmit={handleSubmit(editData)}>
                            <div className="p-2 flex flex-col gap-5">
                                <div className="border border-gray-300 w-full p-3 flex flex-col gap-3 text-sm">
                                    ...
                                    <div className="flex flex-row w-full justify-between items-center gap-2">
                                        <label className="w-1/4">Nama Gudang : </label>
