...
export default function Operator() {
    const {setOperator, listOperator} = customerState()
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/operator', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/operator/' + e);
            showSuccessToast("Sukses Hapus Data");
        } catch (e) {
            showErrorToast("Gagal Hapus Data");
        } finally {
            setCloseModal(false);
            await fetchDataOperator();
        }
    };

    const editData = async (data) => {
        try {
            await axios.put('/api/operator/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        ...
        return listOperator.filter((item) => {
            for (let key in item) {
                        ...
                        <form onSubmit={handleSubmit(editData)}>
                            <div className="p-2 flex flex-col gap-5">
                                <div className="border border-gray-300 w-full p-3 flex flex-col gap-3 text-sm">
                                    ...
                                    <div className="flex flex-row w-full justify-between items-center gap-2">
                                        <label className="w-1/4">Nama Operator : </label>
