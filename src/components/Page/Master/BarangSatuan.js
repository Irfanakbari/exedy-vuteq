...
export default function BarangSatuan() {
    const {setBarangSatuan, listBarangSatuan} = customerState()
    ...
    useEffect(() => {
        fetchDataBarangSatuan();
    }, []);

    const fetchDataBarangSatuan = async () => {
        try {
            const response = await axios.get('/api/barang/satuan');
            setBarangSatuan(response.data['data']);
    ...
    const submitData = async (data) => {
        try {
            await axios.post('/api/barang/satuan', data).then(r => {
    ...
    const deleteData = async (e) => {
        try {
            await axios.delete('/api/barang/satuan/' + e);
    ...
    const editData = async (data) => {
        try {
            await axios.put('/api/barang/satuan/' + selectedCell.kode, data).then(r => {
    ...
    const searchValue = (value) => {
        if (value.trim() === '') {
            return listBarangSatuan;
        }

        const searchValueLowerCase = value.toLowerCase();

        return listBarangSatuan.filter((item) => {
            for (let key in item) {
